...
export default function BirthdaysPage() {
  ...
  const months = t.birthdays.months as string[];
  const days   = t.birthdays.days as string[];
    ...
    <>
      ...
      <div className="page-content" style={{ maxWidth: '900px' }}>
        ...
        <div style={{ marginBottom: '28px' }}>
          <h1 style={{ margin: 0, fontSize: '26px', fontWeight: 800, color: '#1a1a1a', letterSpacing: '-0.04em' }}>{t.birthdays.title}</h1>
          <p style={{ margin: '5px 0 0', fontSize: '13px', color: '#9ca3af' }}>{t.birthdays.subtitle}</p>
        </div>
        ...
        <section style={{ background: '#fff', borderRadius: '18px', padding: '24px', border: '1px solid #f1f5f9', boxShadow: '0 1px 6px rgba(0,0,0,0.05)', marginBottom: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '18px' }}>
            ...
            <span style={{ fontSize: '15px', fontWeight: 700, color: '#1a1a1a' }}>{t.birthdays.upcomingSection}</span>
          </div>
          ...
          <div className="tab-scroll" style={{ display: 'flex', gap: '6px', marginBottom: '16px', background: '#f1f5f9', borderRadius: '10px', padding: '4px', width: 'fit-content' }}>
            {[
              { key: 'this', label: `${months[thisMonthNum - 1]} ${t.birthdays.thisMonth}` },
              { key: 'next', label: `${months[nextMonthNum - 1]} ${t.birthdays.nextMonth}` },
            ].map(tabItem => (
              <button key={tabItem.key} onClick={() => setTab(tabItem.key as 'this' | 'next')}
                style={{ padding: '7px 16px', borderRadius: '7px', border: 'none', fontFamily: 'inherit', fontSize: '13px', fontWeight: tab === tabItem.key ? 700 : 500, cursor: 'pointer', background: tab === tabItem.key ? '#fff' : 'transparent', color: tab === tabItem.key ? '#f59e0b' : '#6b7280', boxShadow: tab === tabItem.key ? '0 1px 4px rgba(0,0,0,0.08)' : 'none', transition: 'all 0.15s' }}>
                {tabItem.label}
              </button>
            ))}
          </div>
          ...
          {loading ? (
            <div style={{ textAlign: 'center', padding: '32px', color: '#9ca3af', fontSize: '13px' }}>{t.common.loading}</div>
          ) : displayed.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '32px', background: '#f9fafb', borderRadius: '12px', border: '1.5px dashed #e5e7eb' }}>
              <div style={{ fontSize: '36px', marginBottom: '8px' }}>🎂</div>
              <div style={{ fontSize: '14px', fontWeight: 600, color: '#374151', marginBottom: '4px' }}>{t.birthdays.noBirthdays}</div>
              <div style={{ fontSize: '13px', color: '#9ca3af' }}>{t.birthdays.noBirthdaysHint}</div>
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: '12px', animation: 'fadeIn 0.2s' }}>
              {displayed.map(m => {
                ...
                const mmNum = parseInt(m.birth_date.slice(5, 7), 10);
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '9px' }}>
                        ...
                        <div>
                          ...
                          {m.age && <div style={{ fontSize: '11px', color: '#9ca3af' }}>{t.birthdays.ageLabel.replace('{n}', String(m.age))}</div>}
                        </div>
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: '13px', color: '#6b7280' }}>🗓 {months[mmNum-1]} {parseInt(dd)}</span>
                      <span style={{
                        ...
                        {isToday ? t.birthdays.todayLabel : `D-${m.days_until}`}
                      </span>
                    </div>
                  </div>
                );
        ...
        <section style={{ background: '#fff', borderRadius: '18px', padding: '24px', border: '1px solid #f1f5f9', boxShadow: '0 1px 6px rgba(0,0,0,0.05)', marginBottom: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '15px', fontWeight: 700, color: '#1a1a1a' }}>{t.birthdays.calendarSection}</span>
            </div>
            {/* 월 이동 */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {Array.from({ length: daysInCal }).map((_, i) => {
              ...
              return (
                <div key={day} className={hasBday ? 'cal-day' : ''}
                  style={{ minHeight: '52px', borderRadius: '8px', padding: '4px 5px', background: isToday ? '#fdf8e8' : hasBday ? '#fffbeb' : 'transparent', border: isToday ? '1.5px solid #f0d88a' : hasBday ? '1px solid #fcd34d' : '1px solid transparent', transition: 'background 0.12s' }}>
                  ...
                  {bdays.length > 2 && <div style={{ fontSize: '9px', color: '#9ca3af' }}>+{bdays.length - 2}</div>}
                </div>
              );
            })}
          </div>
        ...
        <section style={{ background: '#fff', borderRadius: '18px', padding: '24px', border: '1px solid #f1f5f9', boxShadow: '0 1px 6px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '15px', fontWeight: 700, color: '#1a1a1a' }}>{t.birthdays.settingsSection}</span>
            </div>
            {settAlert && (
              <div style={{ padding: '6px 14px', borderRadius: '8px', background: settAlert === 'ok' ? '#f0fdf4' : '#fef2f2', border: `1px solid ${settAlert === 'ok' ? '#86efac' : '#fca5a5'}`, color: settAlert === 'ok' ? '#16a34a' : '#dc2626', fontSize: '12px', fontWeight: 700 }}>
                {settAlert === 'ok' ? t.birthdays.savedOk : t.birthdays.saveFailed}
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderRadius: '12px', background: '#f9fafb', border: '1px solid #f1f5f9' }}>
              <div>
                <div style={{ fontSize: '14px', fontWeight: 700, color: '#1a1a1a' }}>{t.birthdays.enableAlerts}</div>
                <div style={{ fontSize: '12px', color: '#9ca3af', marginTop: '2px' }}>{t.birthdays.enableAlertsDesc}</div>
              </div>
            ...
            <div style={{ padding: '14px 18px', borderRadius: '12px', background: '#f9fafb', border: '1px solid #f1f5f9', opacity: settings.is_active ? 1 : 0.4, pointerEvents: settings.is_active ? 'auto' : 'none' }}>
              <div style={{ fontSize: '14px', fontWeight: 700, color: '#1a1a1a', marginBottom: '12px' }}>
                {t.birthdays.alertDaysLabel}
                <span style={{ fontSize: '13px', fontWeight: 400, color: '#9ca3af', marginLeft: '8px' }}>{t.birthdays.currentSetting.replace('{n}', String(settings.alert_days_before))}</span>
              </div>
                ...
                {ALERT_DAYS_OPTIONS.map(d => {
                  ...
                  return (
                    <button key={d} onClick={() => setSettings(p => ({ ...p, alert_days_before: d }))}
                      ...
                      {t.birthdays.daysBefore.replace('{n}', String(d))}
                    </button>
                  );
                })}
              </div>
            ...
            <div style={{ padding: '14px 18px', borderRadius: '12px', background: '#f9fafb', border: '1px solid #f1f5f9', opacity: settings.is_active ? 1 : 0.4 }}>
              <div style={{ fontSize: '14px', fontWeight: 700, color: '#1a1a1a', marginBottom: '4px' }}>{t.birthdays.alertMethod}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', borderRadius: '9px', background: '#fdf8e8', border: '1.5px solid #f0d88a' }}>
                ...
                <div>
                  <div style={{ fontSize: '13px', fontWeight: 700, color: '#c9a84c' }}>{t.birthdays.appNotification}</div>
                  <div style={{ fontSize: '11px', color: '#d4b85c', marginTop: '1px' }}>{t.birthdays.appNotificationDesc}</div>
                </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button onClick={saveSettings} disabled={settSaving}
                ...
                {settSaving ? t.birthdays.saving : t.birthdays.saveSettings}
              </button>
            </div>
          </div>
        </section>
