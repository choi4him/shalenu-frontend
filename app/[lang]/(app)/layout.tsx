...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // Strip /{lang} prefix for nav matching
  const cleanPath = useMemo(() => pathname.replace(/^\/(ko|en)/, '') || '/', [pathname]);
  ...
  const getInitialOpen = () => {
    ...
    NAV_ITEMS.forEach(item => {
      if (item.children) {
        const isGroupActive = item.children.some(c => cleanPath.startsWith(c.href));
  ...
  useEffect(() => {
    setOpenGroups(prev => {
      ...
      NAV_ITEMS.forEach(item => {
        if (item.children) {
          if (item.children.some(c => cleanPath.startsWith(c.href))) {
        ...
        <div className="plan-limit-banner" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontSize: '14px', fontWeight: 600 }}>
              ...
              <a href={`/${lang}/settings?tab=plan`} style={{ fontWeight: 800, textDecoration: 'underline', color: '#1a1208' }}>
                설정 &gt; 요금제에서 업그레이드하세요.
              </a>
            </span>
          </div>
                    ...
                    <div className="sub-list" style={{ paddingLeft:'14px', marginTop:'2px', display:'flex', flexDirection:'column', gap:'1px' }}>
                      ...
                      <div style={{ borderLeft:'1.5px solid rgba(201,168,76,0.25)', paddingLeft:'10px', display:'flex', flexDirection:'column', gap:'1px' }}>
                        {item.children.map(sub => {
                          const isSubActive = cleanPath === sub.href || cleanPath.startsWith(sub.href + '/');
