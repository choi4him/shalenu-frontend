...
function AddTypeModal({ onClose, onAdded }: { onClose: () => void; onAdded: (name: string) => void }) {
  ...
  const submit = async () => {
    ...
    if (!trimmed) { setError('유형 이름을 입력해주세요.'); return; }
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:1100, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.35)', backdropFilter:'blur(3px)' }}
      ...
      <div style={{ background:'#fff', borderRadius:'16px', padding:'24px', width:'340px', maxWidth:'95vw', boxShadow:'0 16px 48px rgba(0,0,0,0.18)' }}>
        ...
        <input style={inputSt} placeholder="예) 속회, 청년부, 성가대" value={name}
...
function CreateModal({ types, typeStyles, onClose, onCreated }: {
  ...
  const [form, setForm] = useState({ name: '', type: types[0] ?? '구역', meeting_day: '', meeting_time: '' });
  ...
  const submit = async () => {
    if (!form.name.trim()) { setError('소그룹명을 입력해주세요.'); return; }
    setSaving(true); setError('');
    try {
      ...
      <div style={{ position:'fixed', inset:0, zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.4)', backdropFilter:'blur(3px)' }}
        ...
        <div style={{ background:'#fff', borderRadius:'20px', padding:'32px', width:'500px', maxWidth:'95vw', boxShadow:'0 20px 60px rgba(0,0,0,0.2)', maxHeight:'90vh', overflowY:'auto' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
            <div>
              <label style={labelSt}>소그룹명 <span style={{ color:'#ef4444' }}>*</span></label>
            ...
            <div>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'6px' }}>
                <label style={{ ...labelSt, marginBottom:0 }}>유형</label>
              ...
              <div style={{ display:'flex', gap:'7px', flexWrap:'wrap' }}>
                {localTypes.map((t, i) => {
                  const c = typeStyles[t] ?? getTypeStyle(t, i);
            ...
            <div>
              <label style={labelSt}>리더</label>
              <input style={inputSt} placeholder="리더 이름 검색..." value={leaderName || memberSearch}
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
              <div>
                <label style={labelSt}>모임 요일</label>
              ...
              <div>
                <label style={labelSt}>모임 시간</label>
    ...
    <div onClick={onClick}
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'5px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'6px', fontSize:'12px', color:'#6b7280' }}>
          ...
          <span>리더: <strong style={{ color:'#374151' }}>{group.leader_name ?? '미지정'}</strong></span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:'6px', fontSize:'12px', color:'#6b7280' }}>
          ...
          <span>소그룹원 <strong style={{ color:'#374151' }}>{group.member_count}</strong>명</span>
        </div>
        {group.meeting_day && (
          <div style={{ display:'flex', alignItems:'center', gap:'6px', fontSize:'12px', color:'#6b7280' }}>
            ...
            <span>{group.meeting_day}요일 {group.meeting_time ?? ''}</span>
          </div>
        )}
      </div>
    </div>
...
export default function GroupsPage() {
  ...
  const loadTypes = useCallback(async () => {
    try {
      ...
      types.forEach((t, i) => { styles[t] = getTypeStyle(t, i); });
      setTypeStyles(styles);
    } catch {
      setGroupTypes(DEFAULT_TYPES);
      const styles: Record<string, ReturnType<typeof getTypeStyle>> = {};
      DEFAULT_TYPES.forEach((t, i) => { styles[t] = getTypeStyle(t, i); });
      setTypeStyles(styles);
    }
  }, []);
  ...
  const displayed = filter === '전체' ? groups : groups.filter(g => g.type === filter);

  // 표시되는 그룹의 유형별 인덱스
  const allTypes = [...new Set(groups.map(g => g.type))];
  const typeIndexMap = Object.fromEntries(allTypes.map((t, i) => [t, groupTypes.indexOf(t) !== -1 ? groupTypes.indexOf(t) : i]));
    ...
    <>
      ...
      <div style={{ padding:'36px 40px', maxWidth:'1000px' }}>
        ...
        <div style={{ display:'flex', gap:'6px', marginBottom:'24px', flexWrap:'wrap' }}>
          ...
          {groupTypes.map((t, i) => {
            const c = typeStyles[t] ?? getTypeStyle(t, i);
            const cnt = groups.filter(g => g.type === t).length;
            ...
            return (
              <button key={t} onClick={() => setFilter(t)}
                ...
                <span style={{ marginRight:'4px' }}>{getTypeIcon(t)}</span>{t}
