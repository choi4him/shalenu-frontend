...
function BookingModal({
  ...
  const save = async () => {
    if (!selectedFacId)       { setErr(t.facilities.errFacility); return; }
    if (!form.title.trim())   { setErr(t.facilities.errBookingTitle); return; }
    if (form.start_time >= form.end_time) { setErr(t.facilities.errTimeOrder); return; }
    ...
    try {
      ...
      setErr(e instanceof Error ? e.message : t.facilities.errSubmit);
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div style={{ background: '#fff', borderRadius: '20px', padding: '28px', width: '480px', maxWidth: '95vw', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '22px' }}>
          <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 800, color: '#1a1a1a' }}>{t.facilities.bookingModalTitle}</h3>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          {/* 시설 선택 */}
          <div>
            <label style={labelStyle}>{t.facilities.facilityLabel}</label>
            <select value={selectedFacId} onChange={e => setSelectedFacId(e.target.value || '')}
              style={{ ...inputStyle, background: '#fff', cursor: 'pointer' }}>
              <option value="">{t.facilities.selectFacilityPlaceholder}</option>
              {facilities.filter(f => f.is_active).map(f => (
                <option key={f.id} value={f.id}>{f.name}{f.capacity ? ` (${t.facilities.capacity.replace('{n}', String(f.capacity))})` : ''}</option>
              ))}
            </select>
          </div>
          ...
          {/* 예약 제목 */}
          <div>
            <label style={labelStyle}>{t.facilities.bookingTitleLabel}</label>
            <input style={inputStyle} placeholder={t.facilities.bookingTitlePlaceholder} value={form.title}
              onChange={e => setForm(p => ({ ...p, title: e.target.value }))} />
          </div>

          {/* 날짜 */}
          <div>
            <label style={labelStyle}>{t.facilities.dateLabel}</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <div>
              <label style={labelStyle}>{t.facilities.startTimeLabel}</label>
              <input type="time" style={inputStyle} value={form.start_time}
                onChange={e => setForm(p => ({ ...p, start_time: e.target.value }))} />
            </div>
            <div>
              <label style={labelStyle}>{t.facilities.endTimeLabel}</label>
          ...
          {/* 메모 */}
          <div>
            <label style={labelStyle}>{t.facilities.noteLabel}</label>
            <textarea style={{ ...inputStyle, minHeight: '80px', resize: 'vertical' }} placeholder={t.facilities.notePlaceholder}
              value={form.note} onChange={e => setForm(p => ({ ...p, note: e.target.value }))} />
          </div>

          <div style={{ background: '#f0fdf4', borderRadius: '9px', padding: '10px 12px', border: '1px solid #86efac' }}>
            <span style={{ fontSize: '12px', color: '#16a34a', fontWeight: 600 }}>{t.facilities.approvalNotice}</span>
          </div>

          {/* 버튼 */}
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', paddingTop: '4px' }}>
            <button onClick={onClose} style={{ padding: '10px 18px', borderRadius: '10px', border: '1.5px solid #e5e7eb', background: '#fff', color: '#374151', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>{t.common.cancel}</button>
            <button onClick={save} disabled={saving}
              ...
              {saving ? t.facilities.submitting : t.facilities.submitBooking}
            </button>
          </div>
        </div>
      </div>
...
function FacilityModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const save = async () => {
    if (!form.name.trim()) { setErr(t.facilities.errFacilityName); return; }
    setSaving(true); setErr('');
    try {
      ...
      onDone();
    } catch (e) { setErr(e instanceof Error ? e.message : t.facilities.registerFailed); }
    finally { setSaving(false); }
  };
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div style={{ background: '#fff', borderRadius: '20px', padding: '28px', width: '420px', maxWidth: '95vw', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
          <h3 style={{ margin: 0, fontSize: '17px', fontWeight: 800, color: '#1a1a1a' }}>{t.facilities.facilityModalTitle}</h3>
          <button onClick={onClose} style={{ border: 'none', background: '#f3f4f6', borderRadius: '8px', padding: '7px', cursor: 'pointer', display: 'flex' }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6b7280" strokeWidth="2.5" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div><label style={labelStyle}>{t.facilities.facilityNameLabel}</label><input style={inputStyle} placeholder={t.facilities.facilityNamePlaceholder} value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} /></div>
          <div><label style={labelStyle}>{t.facilities.capacityLabel}</label><input type="number" style={inputStyle} placeholder={t.facilities.capacityPlaceholder} value={form.capacity} onChange={e => setForm(p => ({ ...p, capacity: e.target.value }))} /></div>
          <div><label style={labelStyle}>{t.facilities.descriptionLabel}</label><textarea style={{ ...inputStyle, minHeight: '70px', resize: 'vertical' }} placeholder={t.facilities.descriptionPlaceholder} value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} /></div>
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
            <button onClick={onClose} style={{ padding: '9px 16px', borderRadius: '9px', border: '1.5px solid #e5e7eb', background: '#fff', color: '#374151', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>{t.common.cancel}</button>
            <button onClick={save} disabled={saving} style={{ padding: '9px 20px', borderRadius: '9px', border: 'none', background: saving ? '#f0d88a' : '#c9a84c', color: saving ? '#d4b85c' : '#fff', fontSize: '13px', fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>{saving ? t.common.saving : t.facilities.registerFacility}</button>
          </div>
        </div>
      </div>
    </div>
...
export default function FacilitiesPage() {
  ...
  const statusLabels = t.facilities.statusLabels as Record<string, string>;
  const days   = t.facilities.days   as string[];
  const months = t.facilities.months as string[];
    ...
    <>
      ...
      <div className="page-content" style={{ maxWidth: '960px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '26px', fontWeight: 800, color: '#1a1a1a', letterSpacing: '-0.04em' }}>{t.facilities.title}</h1>
            <p style={{ margin: '5px 0 0', fontSize: '13px', color: '#9ca3af' }}>{t.facilities.subtitle}</p>
          </div>
        ...
        <div style={{ background: '#fff', borderRadius: '18px', padding: '22px 24px', border: '1px solid #f1f5f9', boxShadow: '0 1px 6px rgba(0,0,0,0.05)', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '15px', fontWeight: 700, color: '#1a1a1a' }}>{t.facilities.calendarTitle}</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '14px', fontWeight: 700, color: '#1a1a1a', minWidth: '90px', textAlign: 'center' }}>{months[calMonth]} {calYear}</span>
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div key={day} className="cal-day-btn"
                  ...
                  {approvedCount > 0 && <div style={{ fontSize: '9px', fontWeight: 700, padding: '1px 4px', borderRadius: '4px', background: isSel ? 'rgba(255,255,255,0.25)' : '#f0fdf4', color: isSel ? '#fff' : '#16a34a', marginBottom: '1px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>✓ {approvedCount}</div>}
                  {pendingCount  > 0 && <div style={{ fontSize: '9px', fontWeight: 700, padding: '1px 4px', borderRadius: '4px', background: isSel ? 'rgba(255,255,255,0.25)' : '#fffbeb', color: isSel ? '#fff' : '#d97706', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>⏳ {pendingCount}</div>}
                </div>
              );
            })}
          </div>
          ...
          {selectedDay && (
            <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #f1f5f9', animation: 'fadeIn 0.2s' }}>
              <div style={{ fontSize: '13px', fontWeight: 700, color: '#374151', marginBottom: '10px' }}>
                {months[calMonth]} {selectedDay} ({dayBookings.length})
              </div>
              {dayBookings.length === 0 ? (
                <div style={{ fontSize: '13px', color: '#9ca3af', padding: '8px 0' }}>{t.facilities.noBookingsOnDay}</div>
              ) : (
                  ...
                  {dayBookings.map(b => {
                    ...
                    return (
                      <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 12px', borderRadius: '9px', background: '#f9fafb', border: '1px solid #f1f5f9' }}>
                        ...
                        <span style={{ padding: '2px 8px', borderRadius: '10px', background: colors.bg, color: colors.color, border: `1px solid ${colors.border}`, fontSize: '11px', fontWeight: 700 }}>{label}</span>
                      </div>
                    );
                  })}
                </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
          ...
          <span style={{ fontSize: '15px', fontWeight: 700, color: '#1a1a1a' }}>{t.facilities.facilityList}</span>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '56px', background: '#f9fafb', borderRadius: '16px', border: '1.5px dashed #e5e7eb' }}>
            ...
            <div style={{ fontSize: '15px', fontWeight: 600, color: '#374151', marginBottom: '4px' }}>{t.facilities.noFacilities}</div>
            <div style={{ fontSize: '13px', color: '#9ca3af' }}>{t.facilities.noFacilitiesHint}</div>
          </div>
        ) : (
              ...
              return (
                <div key={fac.id} className="fac-card"
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <div>
                        ...
                        {fac.capacity && <div style={{ fontSize: '12px', color: '#9ca3af', marginTop: '2px' }}>{t.facilities.capacity.replace('{n}', String(fac.capacity))}</div>}
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ padding: '8px 10px', borderRadius: '8px', background: '#f8fafc', marginBottom: '12px', border: '1px solid #f1f5f9' }}>
                    <div style={{ fontSize: '11px', fontWeight: 700, color: '#9ca3af', marginBottom: '4px' }}>{t.facilities.todayBookings}</div>
                    {todayFacBk.length === 0 ? (
                      <div style={{ fontSize: '12px', color: '#9ca3af' }}>{t.facilities.noBookingsToday}</div>
                      ...
                      return (
                        <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#374151', marginBottom: '2px' }}>
                          ...
                          <span style={{ fontSize: '10px', padding: '1px 5px', borderRadius: '6px', background: colors.bg, color: colors.color, fontWeight: 700 }}>{label}</span>
                        </div>
                      );
                    })}
                    {todayFacBk.length > 2 && <div style={{ fontSize: '11px', color: '#9ca3af' }}>{t.facilities.moreBookings.replace('{n}', String(todayFacBk.length - 2))}</div>}
                  </div>
