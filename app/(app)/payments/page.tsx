...
function formatAmount(amount: number, currency: string): string {
  ...
  if (cur === 'KRW') return amount.toLocaleString() + '₩';
...
function CreateLinkModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const save = async () => {
    if (!form.title.trim()) { setErr(t.payments.errName); return; }
    if (form.amountType === 'fixed' && (!form.amountValue || Number(form.amountValue) <= 0)) {
      setErr(t.payments.errAmount); return;
    }
    ...
    try {
      ...
      setErr(e instanceof Error ? e.message : t.payments.errCreate);
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div style={{ background: '#fff', borderRadius: '20px', padding: '28px', width: '480px', maxWidth: '95vw', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '22px' }}>
          <div>
            <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 800, color: '#1a1a1a' }}>{t.payments.modalTitle}</h3>
            <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#9ca3af' }}>
              {isKrw ? t.payments.portoneSubtitle : t.payments.stripeSubtitle}
            </p>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          {/* 결제 통화 선택 */}
          <div>
            <label style={labelStyle}>{t.payments.currencyLabel}</label>
          ...
          <div>
            <label style={labelStyle}>{t.payments.nameLabel}</label>
            <input style={inputStyle} placeholder={t.payments.namePlaceholder} value={form.title}
              onChange={e => setForm(p => ({ ...p, title: e.target.value }))} />
          </div>

          <div>
            <label style={labelStyle}>{t.payments.descLabel}</label>
            <textarea style={{ ...inputStyle, minHeight: '70px', resize: 'vertical' }} placeholder={t.payments.descPlaceholder}
              value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} />
          </div>

          <div>
            <label style={labelStyle}>{t.payments.amountTypeLabel}</label>
            <div style={{ display: 'flex', gap: '8px' }}>
              {[
                { key: 'fixed',  label: t.payments.fixedAmount },
                { key: 'custom', label: t.payments.customAmountLabel },
              ].map(opt => (
          ...
          {form.amountType === 'fixed' && (
            <div>
              <label style={labelStyle}>{isKrw ? t.payments.amountKrw : t.payments.amountUsd}</label>
          ...
          {form.amountType === 'custom' && (
            <div style={{ padding: '10px 12px', borderRadius: '9px', background: '#f0f9ff', border: '1px solid #bae6fd' }}>
              <span style={{ fontSize: '12px', color: '#0284c7', fontWeight: 600 }}>{t.payments.customAmountNotice}</span>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', paddingTop: '4px' }}>
            ...
            <button onClick={save} disabled={saving}
              ...
              {saving ? t.payments.creating : t.payments.createBtn}
            </button>
          </div>
        </div>
      </div>
...
export default function PaymentsPage() {
  ...
  const toggleLink = async (id: string) => {
    try {
      ...
      setAlert({ type: 'ok', msg: t.payments.statusChanged });
      await load();
    } catch {
      setAlert({ type: 'err', msg: t.payments.statusChangeFailed });
    }
  ...
  const STATUS_COLORS_CFG: Record<string, { color: string; bg: string }> = {
    completed: { color: '#16a34a', bg: '#f0fdf4' },
    pending:   { color: '#d97706', bg: '#fffbeb' },
    failed:    { color: '#dc2626', bg: '#fef2f2' },
    refunded:  { color: '#7c3aed', bg: '#f5f3ff' },
  };
  const statusLabels = t.payments.statusLabels as Record<string, string>;

  const historyHeaders = t.payments.historyHeaders as string[];
    ...
    <>
      ...
      <div className="page-content" style={{ maxWidth: '960px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '26px', fontWeight: 800, color: '#1a1a1a', letterSpacing: '-0.04em' }}>{t.payments.title}</h1>
            <p style={{ margin: '5px 0 0', fontSize: '13px', color: '#9ca3af' }}>{t.payments.subtitle}</p>
          </div>
        ...
        {stripeErr && (
          <div style={{ padding: '14px 18px', borderRadius: '12px', background: '#fffbeb', border: '1.5px solid #fcd34d', marginBottom: '20px', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
            ...
            <div>
              <div style={{ fontSize: '14px', fontWeight: 700, color: '#92400e', marginBottom: '4px' }}>{t.payments.stripeSetupTitle}</div>
              <div style={{ fontSize: '13px', color: '#78350f', lineHeight: 1.6 }}>
                {t.payments.stripeSetupHint}<br />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))', gap: '12px', marginBottom: '24px' }}>
          {[
            { label: t.payments.linksTab, value: t.payments.activeLinksCount.replace('{n}', String(links.filter(l => l.is_active).length)), icon: '🔗', color: '#c9a84c', bg: '#fdf8e8', border: '#f0d88a' },
            { label: t.payments.completedCount, value: String(totalCompleted.length), icon: '✅', color: '#16a34a', bg: '#f0fdf4', border: '#86efac' },
            { label: t.payments.totalAmount, value: totalCompleted.length > 0 ? formatAmount(totalAmount, 'usd') : '—', icon: '💰', color: '#d97706', bg: '#fffbeb', border: '#fcd34d' },
          ].map(s => (
        ...
        <div style={{ display: 'flex', gap: '4px', background: '#f1f5f9', padding: '4px', borderRadius: '10px', marginBottom: '18px', width: 'fit-content' }}>
          {[
            { key: 'links',   label: t.payments.linksTab },
            { key: 'history', label: t.payments.historyTab },
          ].map(tabItem => (
            <button key={tabItem.key} onClick={() => setTab(tabItem.key as any)}
              style={{ padding: '7px 20px', borderRadius: '7px', border: 'none', fontFamily: 'inherit', fontSize: '13px', fontWeight: tab === tabItem.key ? 700 : 500, cursor: 'pointer', background: tab === tabItem.key ? '#fff' : 'transparent', color: tab === tabItem.key ? '#c9a84c' : '#6b7280', boxShadow: tab === tabItem.key ? '0 1px 4px rgba(0,0,0,0.08)' : 'none', transition: 'all 0.13s' }}>
              {tabItem.label}
            </button>
          ))}
        </div>
        ...
        {tab === 'links' && (
          loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '64px', background: '#f9fafb', borderRadius: '16px', border: '1.5px dashed #e5e7eb' }}>
              ...
              <div style={{ fontSize: '15px', fontWeight: 700, color: '#374151', marginBottom: '6px' }}>{t.payments.noLinks}</div>
              <div style={{ fontSize: '13px', color: '#9ca3af', marginBottom: '16px' }}>{t.payments.noLinksHint}</div>
            </div>
          ) : (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                    ...
                    <div style={{ flex: 1, minWidth: '180px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                        ...
                        <span style={{ padding: '2px 8px', borderRadius: '8px', fontSize: '10px', fontWeight: 700, background: link.is_active ? '#f0fdf4' : '#f3f4f6', color: link.is_active ? '#16a34a' : '#9ca3af' }}>
                          {link.is_active ? t.payments.active : t.payments.inactive}
                        </span>
                      </div>
                      ...
                      <div style={{ fontSize: '13px', color: '#c9a84c', fontWeight: 700 }}>
                        {link.amount ? formatAmount(link.amount, link.currency) : t.payments.freeAmount}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '7px', flexShrink: 0, flexWrap: 'wrap', alignItems: 'center' }}>
                        ...
                        return (
                          <>
                            <button onClick={() => copyLink(url, link.id)}
                              ...
                              {copied === link.id ? (
                                <><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg> {t.payments.copied}</>
                              ) : (
                                <><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg> {t.payments.copyLink}</>
                              )}
                            </button>
                      ...
                      <button onClick={() => toggleLink(link.id)}
                        ...
                        {link.is_active ? t.payments.deactivate : t.payments.activate}
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #f1f5f9', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
              ...
              {loading ? (
                ...
                <div style={{ textAlign: 'center', padding: '56px' }}>
                  ...
                  <div style={{ fontSize: '14px', fontWeight: 600, color: '#374151', marginBottom: '4px' }}>{t.payments.noPayments}</div>
                  <div style={{ fontSize: '13px', color: '#9ca3af' }}>{t.payments.noPaymentsHint}</div>
                </div>
              ) : (
                payments.map(p => {
                  const colorCfg = STATUS_COLORS_CFG[p.status] ?? STATUS_COLORS_CFG.pending;
                  ...
                  return (
                    <div key={p.id} className="pay-row"
                      ...
                      <span style={{ fontSize: '12px', color: '#6b7280' }}>
                        {new Date(p.paid_at ?? p.created_at).toLocaleDateString(undefined, { month: '2-digit', day: '2-digit' })}
                      </span>
                      <div style={{ overflow: 'hidden' }}>
                        <div style={{ fontSize: '13px', fontWeight: 600, color: '#1a1a1a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {p.donor_name ?? t.payments.anonymous}
                        </div>
                      ...
                      <span style={{ padding: '3px 8px', borderRadius: '10px', background: colorCfg.bg, color: colorCfg.color, fontSize: '11px', fontWeight: 700, display: 'inline-block' }}>{stLabel}</span>
                    </div>
                  );
                })
              )}
